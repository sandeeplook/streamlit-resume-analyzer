...
Single-process app: the UI, file handling, text extraction, and the Claude
API call all run inside this one Streamlit script. No database, no login —
everything happens in memory for the current session only.
"""
...
if not settings.anthropic_api_key:
    st.warning(
        "No `ANTHROPIC_API_KEY` is configured. Add it under **Settings → Secrets** "
        "in Streamlit Community Cloud, or as an environment variable locally, "
...
if st.session_state.analysis_data is None:
    ...
    st.caption(
        "Upload a resume and provide the job description. Claude cross-checks the "
        "resume for consistency and scores its fit against the role."
    )
    ...
    if run_clicked:
        ...
        if resume_file is None:
            ...
            try:
                ...
                analysis = analyze_resume(settings, resume_text, jd_clean)
...
st.write("")
st.caption("Analysis generated by Claude · Not a substitute for independent background verification.")
